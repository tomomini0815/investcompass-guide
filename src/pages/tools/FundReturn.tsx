...
import { ArrowLeft, PieChart, Lightbulb, LineChart } from "lucide-react";
...
const FundReturn = () => {
  ...
  const [assetValueChange, setAssetValueChange] = useState(0);
  const [dividends, setDividends] = useState(0);
  const [expectedReturn, setExpectedReturn] = useState(0);

  const calculate = () => {
    const purchase = parseFloat(purchaseNav) || 0;
    const sell = parseFloat(sellNav) || 0;
    const unitCount = parseFloat(units) || 0;
    const dividend = parseFloat(dividendPerUnit) || 0;
    
    const valueChange = (sell - purchase) * unitCount;
    const totalDividends = dividend * unitCount;
    const investment = purchase * unitCount;
    const returnPercent = investment !== 0 ? ((valueChange + totalDividends) / investment) * 100 : 0;
    
    setAssetValueChange(valueChange);
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1">
              ...
              <Card className="overflow-hidden border-2 hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 bg-gradient-to-br from-card to-card/50">
                <div className="md:flex">
                  ...
                  <div className="md:w-2/3 p-8">
                    <h2 className="text-2xl font-bold mb-4 bg-gradient-to-r from-foreground to-foreground/70 bg-clip-text">投資信託のリターンとリスクを把握する</h2>
                    <p className="text-muted-foreground mb-4 leading-relaxed">
                      投資信託では、プロの運用者による資産運用により、
                      分散投資されたポートフォリオのリターンを得ます。
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      <Badge variant="outline" className="hover:bg-primary hover:text-primary-foreground transition-colors">リターン計算</Badge>
                      <Badge variant="outline" className="hover:bg-primary hover:text-primary-foreground transition-colors">リスク評価</Badge>
                    </div>
                  </div>
                </div>
              </Card>
            ...
            <Card className="border-2 hover:shadow-2xl transition-all duration-300 bg-gradient-to-br from-card to-card/80 mb-12">
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                  <div>
                    <Label htmlFor="purchaseNav" className="block text-sm font-medium mb-2">
                      購入時基準価格 (円)
                    </Label>
                    <Input
                      id="purchaseNav"
                  ...
                  <div>
                    <Label htmlFor="sellNav" className="block text-sm font-medium mb-2">
                      売却時基準価格 (円)
                    </Label>
                    <Input
                      id="sellNav"
                  ...
                  <div>
                    <Label htmlFor="dividendPerUnit" className="block text-sm font-medium mb-2">
                      1口当たり分配金 (円)
                    </Label>
                    <Input
                      id="dividendPerUnit"
                ...
                {(assetValueChange !== 0 || dividends !== 0) && (
                  <div className="mt-6 p-6 bg-primary/10 rounded-lg">
                    <h3 className="font-bold mb-4 text-center">計算結果</h3>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div className="text-center p-4 bg-background rounded-lg">
                        <p className="text-sm text-muted-foreground">資産価値変動</p>
                        <p className="text-2xl font-bold text-primary">{assetValueChange.toLocaleString()}円</p>
                      </div>
                      <div className="text-center p-4 bg-background rounded-lg">
                        <p className="text-sm text-muted-foreground">分配金</p>
                        <p className="text-2xl font-bold text-primary">{dividends.toLocaleString()}円</p>
                      </div>
                      <div className="text-center p-4 bg-gradient-to-br from-primary/20 to-accent/20 rounded-lg border-2 border-primary/30">
                        ...
                        <p className="text-2xl font-bold text-primary">{expectedReturn.toFixed(2)}%</p>
                      </div>
                    </div>
                  </div>
                )}
        ...
        {/* Fund Return Explanation */}
        <section className="py-16 container mx-auto px-4">
          <div className="max-w-4xl mx-auto">
            <div className="text-center mb-12">
              <h2 className="text-3xl font-bold mb-4">投資信託リターン計算の基本</h2>
              <p className="text-muted-foreground">
                投資信託のリターンは、資産価値の変動と分配金によって構成されます。
              </p>
            </div>
              ...
              <Card className="border-2 hover:shadow-2xl transition-all duration-300 bg-gradient-to-br from-card to-card/80">
                ...
                <CardContent>
                  <p className="text-muted-foreground mb-4">
                    投資信託の基準価格の変動によって得られる利益です。
                  </p>
                  <div className="bg-muted p-4 rounded-lg">
                    <p className="text-center font-mono">
                      資産価値変動 = (売却時基準価格 - 購入時基準価格) × 口数
                    </p>
                  </div>
                </CardContent>
              </Card>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div>
                    ...
                    <div className="bg-muted p-4 rounded-lg mb-4">
                      <p className="text-center font-mono">
                        期待リターン = (資産価値変動 + 分配金) ÷ 投資額 × 100
                      </p>
                    </div>
                    <p className="text-sm text-muted-foreground">
                      期待リターンは、過去の実績やベンチマーク指数などを基に算出します。
                    </p>
                  </div>
              ...
              <Card className="border-2 hover:shadow-2xl transition-all duration-300 bg-gradient-to-br from-card to-card/80">
                ...
                <CardContent>
                  <div className="space-y-6">
                    <div>
                      <h3 className="font-bold mb-2">投資信託情報</h3>
                      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <div className="p-4 bg-primary/10 rounded-lg">
                          <p className="text-sm text-muted-foreground">ファンド名</p>
                          <p className="font-bold">ABCインデックスファンド</p>
                        </div>
                        <div className="p-4 bg-primary/10 rounded-lg">
                          <p className="text-sm text-muted-foreground">購入時基準価格</p>
                          <p className="font-bold">1,500円</p>
                        </div>
                        <div className="p-4 bg-primary/10 rounded-lg">
                          <p className="text-sm text-muted-foreground">売却時基準価格</p>
                          <p className="font-bold">1,800円</p>
                        </div>
                        <div className="p-4 bg-primary/10 rounded-lg">
                          <p className="text-sm text-muted-foreground">口数</p>
                          <p className="font-bold">100口</p>
                        </div>
                      </div>
                    </div>
                    
                    <div>
                      <h3 className="font-bold mb-2">分配金情報</h3>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="p-4 bg-primary/10 rounded-lg">
                          <p className="text-sm text-muted-foreground">1口当たり分配金</p>
                          <p className="font-bold">5円</p>
                        </div>
                        <div className="p-4 bg-primary/10 rounded-lg">
                    ...
                    <div className="p-6 bg-primary/10 rounded-lg">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="text-center p-4 bg-background rounded-lg">
                          <p className="text-sm text-muted-foreground">資産価値変動</p>
                          <p className="text-2xl font-bold text-primary">30,000円</p>
                        </div>
                        <div className="text-center p-4 bg-background rounded-lg">
                          <p className="text-sm text-muted-foreground">分配金</p>
                          <p className="text-2xl font-bold text-primary">500円</p>
                        </div>
                        <div className="text-center p-4 bg-gradient-to-br from-primary/20 to-accent/20 rounded-lg border-2 border-primary/30">
                          <p className="text-sm text-muted-foreground">期待リターン</p>
                          <p className="text-2xl font-bold text-primary">20.3%</p>
                        </div>
                      </div>
                    </div>
                  </div>
            ...
            <div className="max-w-4xl mx-auto">
              <Card className="relative overflow-hidden bg-gradient-to-br from-primary/20 via-secondary/10 to-accent/20 border-2 border-primary/30 hover:shadow-2xl transition-all duration-500 hover:scale-[1.02]">
                ...
                <CardContent className="p-10 text-center relative z-10">
                  ...
                  <h2 className="text-3xl font-bold mb-4 bg-gradient-to-r from-foreground to-foreground/70 bg-clip-text">投資信託のリターンをシミュレーション</h2>
                  <p className="text-lg text-muted-foreground mb-8 max-w-2xl mx-auto leading-relaxed">
                    投資信託リターン計算機を使って、自分の投資計画をシミュレーションし、
                    より良い投資判断を行いましょう。
                  </p>
